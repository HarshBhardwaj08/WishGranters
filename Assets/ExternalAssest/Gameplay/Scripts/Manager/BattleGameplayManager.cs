...
namespace DragonCrashers
{
    ...
    public class BattleGameplayManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (value >= 1.0f)
            {
                if (aliveHeroUnits != null && aliveEnemyUnits.Count > 1 && introCutsceneBehaviour.isTimeLinePlay == true)
                {
                    for (int i = 0; i < heroTeamUnits.Count; i++)
                    {
                       aliveHeroUnits[i].healthBehaviour.ChangeHealth(-1);
                    }
                }
                value = 0;
            }
        ...
        public void LoadSelectedScene(int n)
        {
            NextSceneLoader sceneLoader = new NextSceneLoader();
            sceneLoader.LoadNextScene(n);
        }
    }
